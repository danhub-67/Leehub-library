...
function LeehHub:CreateWindow(config)
	...
	if not hasBackground then AddGlassShine(CloseConfirmPopup, 12) end
	...
	function Window:CreateTab(tabConfig, maybeIcon)
		...
		function Tab:CreateSection(secConfig)
			...
			function Section:CreateTable(tblConfig)
				...
				header.BackgroundColor3 = Elevate(Theme.ElementBackground)
				header.BackgroundTransparency = hasBackground and 0.35 or 0.1
				header.ZIndex = 7
				...
				table.insert(extras, {Kind = "Outline", Object = headerLine})
				...
				local function buildRows()
					...
					for ri, rowData in ipairs(rowsData) do
						...
						rf.Size = UDim2.new(1, 0, 0, 28)
						rf.BackgroundColor3 = Theme.ElementBackground
						rf.BackgroundTransparency = ri % 2 == 0 and (hasBackground and 0.55 or 0.65) or 1
						rf.ZIndex = 7
						rf.Parent = body
