...
LeehHub.ThemeOrder = {"Aurora", "NeonPurple", "NeonCyan", "NeonPink", "NeonLime", "NeonOrange", "Synthwave", "Cyberpunk", "Toxic", "GlassFrost", "GlassObsidian", "GlassRose"}
...
local function ResolveIcon(key)
	...
	if type(key) == "number" then return "rbxassetid://" .. tostring(key) end
	if type(key) == "string" then
		if LeehHub.Icons[key] then return LeehHub.Icons[key] end
		if string.match(key, "^rbxassetid://") or string.match(key, "^%d+$") then
			return NormalizeImageSource(key)
		end
		return NormalizeImageSource(key)
	end
	return ""
...
function LeehHub:CreateWindow(config)
	...
	local resolvedIcon = ResolveIcon(iconKey)
	...
	if resolvedIcon ~= "" then
		...
		TitleIcon.ImageColor3 = Theme.Icon
		TitleIcon.ZIndex = 6
	...
	local FloatStroke = AddStroke(FloatBtn, Theme.Outline, 2, 0.2)
	AddGlassShine(FloatBtn, 14)

	local resolvedOpenIcon = ResolveIcon(openButtonIcon)
	if resolvedOpenIcon ~= "" then
		local fIcon = Instance.new("ImageLabel")
		fIcon.Size = UDim2.fromScale(0.55, 0.55)
	...
	local function UpdateTheme(name, animate)
		...
		local function Apply()
			...
			local openImage = FloatBtn:FindFirstChildOfClass("ImageLabel")
			if openImage then
				openImage.ImageColor3 = T.Icon
			else
				local openText = FloatBtn:FindFirstChildOfClass("TextLabel")
				if openText then openText.TextColor3 = T.Icon end
			end
			if FloatStroke then FloatStroke.Color = T.Outline end
			...
			for _, item in ipairs(Registered) do
				if item.Type == "TabBtn" then
					if item.Active then
						...
						if item.Icon then item.Icon.ImageColor3 = T.Icon end
					else
						...
						if item.Icon then item.Icon.ImageColor3 = T.Placeholder end
					end
				elseif item.Type == "Element" then
					...
					if item.Extras then
						for _, extra in ipairs(item.Extras) do
							...
							elseif extra.Kind == "Icon" then extra.Object.ImageColor3 = T.Icon
			...
			for _, notification in ipairs(ActiveNotifications) do
				if notification.Frame and notification.Frame.Parent then
					...
					if notification.Icon then notification.Icon.ImageColor3 = T.Icon end
	...
	function Window:EditOpenButton(config)
		...
		if config.Icon ~= nil then
			local iconSource = ResolveIcon(config.Icon)
			local image = FloatBtn:FindFirstChildOfClass("ImageLabel")
	...
	function Window:CreateTab(tabConfig, maybeIcon)
		...
		if resolvedTabIcon ~= "" then
			...
			tIcon.ImageColor3 = Theme.Placeholder
			tIcon.ZIndex = 7
			tIcon.Parent = tabBtn
		end
		...
		local function Activate()
			for _, item in ipairs(Registered) do
				if item.Type == "TabBtn" then
					...
					if item.Icon then item.Icon.ImageColor3 = Theme.Placeholder end
				end
			end
			tabData.Active = true
			page.Visible = true
			...
			if tIcon then tIcon.ImageColor3 = Theme.Icon end
		end
